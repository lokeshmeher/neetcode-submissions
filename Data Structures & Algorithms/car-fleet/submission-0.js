class Fleet {
    constructor() {
        this.stack = [];  // time taken by each car in the fleet
        this.time = undefined;
    }

    add(timeTaken) {
        // Time taken by this fleet would be the time taken by the first car in this fleet.
        if (this.stack.length === 0) this.time = timeTaken;
        this.stack.push(timeTaken);
    }
}
...
class Solution {
    ...
    carFleet(target, position, speed) {
        ...
        while (i>=0) {
            let fleet = new Fleet();
            fleet.add(time[i]);  // Add the time taken by the first car
            i--;
            while (i>=0 && time[i] <= fleet.time) {
                fleet.add(time[i]);
                i--;
            }
            fleets.push(fleet);
